...
function_result Am_Lang_RunningProcess_startNative_0(aobject * const this, aobject * command, aobject * workingDir) {
    ...
    // MorphOS's dosextens.h types fh_Port as LONG (the field is
    // documented as "true if interactive" — historically the type
    // was just bool-ish). Cast through; non-NULL still flips
    // IsInteractive(), and the handler port is a valid pointer
    // either way.
    fh_in->fh_Type  = st->handler_port;
    fh_in->fh_Port  = (LONG) st->handler_port;
    fh_in->fh_Arg1  = (LONG) st;
    fh_out->fh_Type = st->handler_port;
    fh_out->fh_Port = (LONG) st->handler_port;
    fh_out->fh_Arg1 = (LONG) st;
    fh_err->fh_Type = st->handler_port;
    fh_err->fh_Port = (LONG) st->handler_port;
