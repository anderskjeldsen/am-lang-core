...
#include <amigaos/Am/Lang/DateTime.h>
...
// We avoid <dos/datetime.h>'s ClockData and Amiga2Date/Date2Amiga because their
// availability varies across AmigaOS toolchains. Instead we use DateStamp() (V1+)
// for the current time and do calendar arithmetic by hand. All math is in UTC;
// AmigaOS classic has no system timezone concept so DateStamp is treated as UTC.
...
// AmigaOS epoch is 1978-01-01; offset is 8 years + 2 leap days = 252,460,800 s.
function_result Am_Lang_DateTime_getMillis_0()
{
	...
	__result.return_value.value.ulong_value =
		(amiga_seconds + 252460800ULL) * 1000ULL + ms_remainder;
...
// the amigaos build doesn't need to drag in stdio formatting.
